...
def export_schema(spark, month_year: str):
    """
    Exports a schema from a Spark DataFrame to a JSON file.

    This function taks a month_year string in the format "YYYY-MM" and saves the schema from this month_year to a JSON file.

    Args:
        month_year (str)
    """
    year = int(month_year[:4])
    path = f"/Volumes/raw/taxi/yellow_taxi/{year}/yellow_tripdata_{month_year}.parquet"

    df = spark.read.parquet(path)
    schema = df.schema.jsonValue()
    with open(f"{month_year}.json", "w") as f:
        json.dump(schema, f)
    return schema
